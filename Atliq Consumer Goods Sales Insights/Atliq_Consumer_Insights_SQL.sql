...
WHERE 
    mc.manufacturing_cost = (SELECT MIN(manufacturing_cost) FROM fact_manufacturing_cost)
UNION
SELECT 
    p.product_code,
    p.product,  -- corrected from "prodcut" to "product"
    mc.manufacturing_cost
FROM 
    dim_product p
JOIN
...
With cte as(
SELECT *,
	CASE WHEN MONTH(DATE)='9' OR MONTH(DATE)='10' OR MONTH(DATE)='11' THEN 'Q1'
		WHEN MONTH(DATE)='12' OR MONTH(DATE)='1' OR MONTH(DATE)='2' THEN 'Q2'
        WHEN MONTH(DATE)='3' OR MONTH(DATE)='4' OR MONTH(DATE)='5' THEN 'Q3'
        WHEN MONTH(DATE)='6' OR MONTH(DATE)='7' OR MONTH(DATE)='8' THEN 'Q4'
	ELSE NULL
    END AS Quarters
FROM
	fact_sales_monthly)
...
GROUP BY
	c.channel) ,
total_sales as(
select sum(gross_sales) as total_sales from cte)
select cte.channel,cte.gross_sales,(cte.gross_sales/total_sales)*100 as percen
from total_sales;
